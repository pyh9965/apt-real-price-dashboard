...
def load_data_from_api(region_code, start_month, end_month, service_key=None):
    ...
    try:
        ...
        def progress_callback(current_month, total_months, current_items):
            progress = current_month / total_months
            progress_bar.progress(progress)
            status_text.text(f"데이터 로딩 중... ({current_month}/{total_months}개월, {current_items}건)")

        df = api.fetch_multiple_months(region_code, start_month, end_month, progress_callback)
        ...
        # 추가 전처리 (기존 앱과 호환성 유지)
        return preprocess_api_data(df)
    except Exception as e:
        st.error(f"API 데이터 로드 중 오류: {str(e)}")
        return None

def preprocess_api_data(df):
    """API 데이터 추가 전처리 (기존 앱 형식과 호환)"""
    if df.empty:
        return df
...
def main():
    ...
    if API_AVAILABLE:
        ...
        data_source = "📁 파일 업로드"
        st.sidebar.warning("API 모듈을 찾을 수 없습니다. 파일 업로드만 가능합니다.")
        ...
        with st.sidebar.expander("🌐 API 설정", expanded=True):
            ...
            if st.button("🔌 API 연결 테스트"):
                with st.spinner("연결 테스트 중..."):
                    test_key = api_key_input if api_key_input else None
                    result = test_api_connection(test_key)
                    if result["success"]:
    ...
    if df is None or len(df) == 0:
        ...
        #### 방법 1: API 조회 (권장)
        1. **API 조회** 탭을 선택합니다.
        2. 원하는 **지역**과 **기간**을 설정합니다.
        3. **데이터 조회** 버튼을 클릭합니다.

        #### 방법 2: 파일 업로드
        1. **파일 업로드** 탭을 선택합니다.
        2. 국토교통부 실거래가 공개시스템에서 다운로드한 **Excel 파일(.xlsx)**을 업로드합니다.
        """)
